...
export const updateUserById = async (req, res, next) => {
    try {
        ...
        // Allows users to update only their own information (except role)
        ...
        if (updates.role && req.user.role !== 'admin') {
            return res.status(403).json({
                error: 'Access denied',
                message: "Only administrator can change user roles",
            });
        }
...
export const deleteUserById = async (req, res, next) => {
    try {
        ...
        if (req.user.role !== 'admin') {
            return res.status(403).json({
                error: "Access denied",
                message: "Only administrator can delete users",
            });
        }
